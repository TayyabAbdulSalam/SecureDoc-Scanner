...
# Gathering SecureDoc Scanner path variable
base_path = open(".path_handler", "r").read().strip()
...
def generate_pdf_report(output_text, file_name="document_analysis_report.pdf"):
    pdf = canvas.Canvas(file_name)
    pdf.setFont("Helvetica", 12)

    # Split the text into lines and add them to the PDF
    y_position = 800  # Starting y position (adjust as needed)
    line_height = 14  # Height of each line

    for line in output_text.split("\n"):
        pdf.drawString(50, y_position, line)  # 50 is the x position, adjust if needed
...
class DocumentAnalyzer:
    def __init__(self, targetFile):
        ...
        self.file_sigs = json.load(open(f"{base_path}{path_seperator}Systems{path_seperator}Multiple{path_seperator}file_sigs.json"))
        self.base64_pattern = r'(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{4})'
        self.mal_code = json.load(open(f"{base_path}{path_seperator}Systems{path_seperator}Multiple{path_seperator}malicious_html_codes.json"))
        self.mal_rtf_code = json.load(open(f"{base_path}{path_seperator}Systems{path_seperator}Multiple{path_seperator}malicious_rtf_codes.json"))
        self.pat_ct = 0
    ...
    def DocumentYara(self):
        ...
        conf.read(f"{base_path}{path_seperator}Systems{path_seperator}Multiple{path_seperator}multiple.conf")
        rule_path = conf["Rule_PATH"]["rulepath"]
        finalpath = f"{base_path}{path_seperator}{rule_path}"
    ...
    def OneNoteAnalysis(self):
        ...
        for key in efs.keys():
            with open(f"securedoc_carved-{key}{efs[key]['extension']}", "wb") as binfile:
                binfile.write(efs[key]["content"])
            binfile.close()
            print(f"[bold magenta]>>>[white] Embedded file saved as: [bold green]securedoc_carved-{key}{efs[key]['extension']}[white]")
                    ...
                    try:
                            ...
                            for categ in self.file_sigs:
                                for pattern in self.file_sigs[categ]["patterns"]:
                                    ...
                                    if regex != []:
                                        ...
                                        print(f"{infoS} Attempting to extraction...")
                                        self.output_writer(out_file=f"securedoc_carved-{categ}-{obj}.bin", mode="wb", buffer=object_data)
                        ...
                        if "EmbeddedFile" in str(doc.getobj(obj)) and "PDFStream" in str(doc.getobj(obj)):
                            ...
                            try:
                                ...
                                self.output_writer(out_file=f"securedoc_embedded_decompressed_file-{obj}.bin", mode="wb", buffer=decompressed)
                            except:
                                self.output_writer(out_file=f"securedoc_embedded_file-{obj}.bin", mode="wb", buffer=doc.getobj(obj).get_rawdata())
                    except:
                        continue
            else:
                pass
        ...
        if self.mal_code["unescape"]["count"] != 0:
            ...
            if un_dat != []:
                for escape in un_dat:
                    ...
                    self.output_writer(out_file=f"securedoc_decoded_unescape-{len(deobf)}.bin", mode="w", buffer=deobf)

                    # After extracting the data also we need to scan it!
                    print(f"\n{infoS} Performing analysis against [bold yellow]securedoc_decoded_unescape-{len(deobf)}.bin[white]")
                    if "html" in deobf:
                        ...
                        if len(decoded.decode()) <= 6 and (")" in decoded.decode() or "(" in decoded.decode() or "[" in decoded.decode() or "]" in decoded.decode() or "+" in decoded.decode() or "-" in decoded.decode() or "<" in decoded.decode() or ">" in decoded.decode() or "*" in decoded.decode() or "!" in decoded.decode()):
                            ...
                            if key_count != 0:
                                if len(decoded.decode()) >= 150:
                                    ...
                                    self.output_writer(out_file=f"securedoc_decoded_javascript-{len(decoded.decode())}.js", mode="w", buffer=decoded.decode())
                                else:
        ...
        if javscr != []:
            ...
            for jv in javscr:
                ...
                if mal_ind != 0 and len(jav_buf) > 0:
                    self.output_writer(out_file=f"securedoc_carved_javascript-{len(jav_buf)}.js", mode="w", buffer=jav_buf)
        else:
        ...
        for exp_pattern in self.rtf_exploit_extract_dict:
            for pattern in self.rtf_exploit_extract_dict[exp_pattern]["pattern"]:
                ...
                if chk_ex != []:
                    ...
                    self.pat_ct += 1
                    self.output_writer(out_file=f"securedoc_extracted_exploit-{len(buffer)}.bin", mode="wb", buffer=binascii.unhexlify(buffer))
    ...
    def rtf_check_exploit_main(self, buffer):
        ...
        if get_brackets != []:
            ...
            self.output_writer(out_file=f"securedoc_extracted_exploit-{len(finalbuffer)}.bin", mode="wb", buffer=finalbuffer)
            ...
            if self.mal_rtf_code["unescape"]["count"] != 0:
                ...
                if unesc != []:
                    ...
                    for un in unesc:
                        ...
                        self.output_writer(out_file=f"securedoc_deobfuscated_unescape-{len(deobf)}.bin", mode="w", buffer=deobf)
        else:
...
try:
    # Initialize the DocumentAnalyzer and run the analysis
    ...
    if ext == "docscan":
        ...
        print(f"{errorS} The document is Non malicious / Clean")
    else:
...
# Get the output text from the buffer
...
if generate_report == 'y':
    if output_text:  # Check if output_text is not empty
        generate_pdf_report(output_text)  # Corrected function call
    else:
